...
#include "iothubtransport_generic_amqp_messenger.h"
...
#define MESSENGER_OPTION_EVENT_SEND_TIMEOUT_SECS        600
#define MESSENGER_OPTION_SAVED_OPTIONS                  "messenger_saved_options"


typedef struct MESSENGER_INSTANCE_TAG
{
    GMESSENGER_CONFIG* config;

    SINGLYLINKEDLIST_HANDLE waiting_to_send;
    SINGLYLINKEDLIST_HANDLE in_progress_list;
    GMESSENGER_STATE state;
    
    ON_GMESSENGER_STATE_CHANGED_CALLBACK on_state_changed_callback;
    void* on_state_changed_context;

    bool receive_messages;
    ON_GMESSENGER_MESSAGE_RECEIVED on_message_received_callback;
...
typedef struct MESSENGER_SEND_EVENT_TASK_TAG
{
    ...
    ON_GMESSENGER_SEND_COMPLETE on_event_send_complete_callback;
    void* context;
    time_t send_time;
...
static int validate_configuration(const GMESSENGER_CONFIG* messenger_config)
{
    int result;

    if (messenger_config == NULL)
    {
        LogError("configuration handle is NULL");
        result = __FAILURE__;
    }
    else if (messenger_config->device_id == NULL || messenger_config->iothub_host_fqdn == NULL ||
        messenger_config->receive_link_relative_path == NULL || messenger_config->send_link_relative_path == NULL)
    {
        LogError("configuration has invalid NULL parameter (device_id=%p, iothub_host_fqdn=%p, receive_link_relative_path=%p, send_link_relative_path=%p)",
            messenger_config->device_id, messenger_config->iothub_host_fqdn, messenger_config->receive_link_relative_path, messenger_config->send_link_relative_path);
        result = __FAILURE__;
    }
    else
    {
...
static void destroy_configuration(GMESSENGER_CONFIG* config)
{
    if (config != NULL)
    {
        ...
        if (config->receive_link_relative_path != NULL)
        {
            free(config->receive_link_relative_path);
        }

        if (config->send_link_relative_path != NULL)
        {
            free(config->send_link_relative_path);
        }

        if (config->receive_link_attach_properties != NULL)
        {
            Map_Destroy(config->receive_link_attach_properties);
        }

        if (config->send_link_attach_properties != NULL)
        {
            Map_Destroy(config->send_link_attach_properties);
        }
...
static GMESSENGER_CONFIG* clone_configuration(const GMESSENGER_CONFIG* src)
{
    GMESSENGER_CONFIG* result;

    if (mallocAndStrcpy_s(result->device_id, src->device_id) != 0)
    {
        LogError("failed cloning configuration (device_id)");
        destroy_configuration(result);
        result = NULL;
    }
    else if (mallocAndStrcpy_s(result->iothub_host_fqdn, src->iothub_host_fqdn) != 0)
    {
        LogError("failed cloning configuration (iothub_host_fqdn)");
        destroy_configuration(result);
        result = NULL;
    }
    else if (mallocAndStrcpy_s(result->receive_link_relative_path, src->receive_link_relative_path) != 0)
    {
        LogError("failed cloning configuration (receive_link_relative_path)");
        destroy_configuration(result);
        result = NULL;
    }
    else if (mallocAndStrcpy_s(result->send_link_relative_path, src->send_link_relative_path) != 0)
    {
        LogError("failed cloning configuration (send_link_relative_path)");
        destroy_configuration(result);
        result = NULL;
    }
    else if ((result->receive_link_attach_properties = Map_Clone(src->receive_link_attach_properties)) == NULL)
    {
        LogError("failed cloning configuration (receive_link_attach_properties)");
        destroy_configuration(result);
        result = NULL;
    }
    else if ((result->send_link_attach_properties = Map_Clone(src->send_link_attach_properties)) == NULL)
    {
        LogError("failed cloning configuration (send_link_attach_properties)");
        destroy_configuration(result);
        result = NULL;
    }
...
static void update_messenger_state(MESSENGER_INSTANCE* instance, GMESSENGER_STATE new_state)
{
    if (new_state != instance->state)
    {
        GMESSENGER_STATE previous_state = instance->state;
...
static int create_event_sender(MESSENGER_INSTANCE* instance)
{
    ...
    // Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_033: [A variable, named `devices_path`, shall be created concatenating `instance->config->iothub_host_fqdn`, "/devices/" and `instance->config->device_id`]
    if ((devices_path = create_devices_path(instance->config->iothub_host_fqdn, instance->config->device_id)) == NULL)
    {
    ...
    else if ((link_name = create_link_name(MESSAGE_SENDER_LINK_NAME_PREFIX, STRING_c_str(instance->config->device_id))) == NULL)
    {
...
static int create_message_receiver(MESSENGER_INSTANCE* instance)
{
    ...
    // Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_068: [A variable, named `devices_path`, shall be created concatenating `instance->config->iothub_host_fqdn`, "/devices/" and `instance->config->device_id`]
    if ((devices_path = create_devices_path(instance->config->iothub_host_fqdn, instance->config->device_id)) == NULL)
    {
    ...
    else if ((link_name = create_link_name(MESSAGE_RECEIVER_LINK_NAME_PREFIX, STRING_c_str(instance->config->device_id))) == NULL)
    {
...
{ 
    if (context != NULL)
    {
        ...
        if (task->messenger->message_sender_current_state != MESSAGE_SENDER_STATE_ERROR)
        {
            ...
            {
                GMESSENGER_SEND_COMPLETE_RESULT messenger_send_result;
                ...
                {
                    messenger_send_result = GMESSENGER_SEND_COMPLETE_RESULT_OK;
                }
                ...
                {
                    messenger_send_result = GMESSENGER_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING;
                }
...
static int send_pending_events(MESSENGER_INSTANCE* instance)
{
    ...
    while ((task = get_next_event_to_send(instance)) != NULL)
    {
        ...
        {
            result = __FAILURE__;
            task->on_event_send_complete_callback(task->message, GMESSENGER_SEND_COMPLETE_RESULT_ERROR_FAIL_SENDING, (void*)task->context);
            break;
        }
        else
        {
            ...
            if ((uamqp_result = message_create_from_iothub_message(task->message->messageHandle, &amqp_message)) != RESULT_OK)
            {
                ...
                task->on_event_send_complete_callback(task->message, GMESSENGER_SEND_COMPLETE_RESULT_ERROR_CANNOT_PARSE, (void*)task->context);
            ...
            if (task->is_timed_out == false)
            {
                ...
                if (is_timeout_reached(task->send_time, instance->event_send_timeout_secs, &is_timed_out) == RESULT_OK)
                {
                    if (is_timed_out)
                    {
                        ...
                        {
                            task->on_event_send_complete_callback(task->message, GMESSENGER_SEND_COMPLETE_RESULT_ERROR_TIMEOUT, task->context);
                        }
                    }
                }
                else
...
int messenger_subscribe_for_messages(GMESSENGER_HANDLE messenger_handle, ON_GMESSENGER_MESSAGE_RECEIVED on_message_received_callback, void* context)
{
    int result;
...
int messenger_send_async(GMESSENGER_HANDLE messenger_handle, IOTHUB_MESSAGE_LIST* message, ON_GMESSENGER_SEND_COMPLETE on_messenger_event_send_complete_callback, void* context)
{
    int result;
...
void messenger_do_work(MESSENGER_HANDLE messenger_handle)
{
    ...
    else
    {
        ...
        else if (instance->state == MESSENGER_STATE_STARTED)
        {
            ...
            {
                LogError("messenger_do_work warning (failed creating the message receiver [%s])", STRING_c_str(instance->config->device_id));
            }
...
void gmessenger_destroy(GMESSENGER_HANDLE messenger_handle)
{
    ...
    else
    {
        ...
        if (instance->state != GMESSENGER_STATE_STOPPED)
        {
        ...
        while ((list_node = singlylinkedlist_get_head_item(instance->in_progress_list)) != NULL)
        {
            ...
            if (task != NULL)
            {
                task->on_event_send_complete_callback(task->message, GGMESSENGER_SEND_COMPLETE_RESULT_MESSENGER_DESTROYED, (void*)task->context);
                free(task);
            }
        }
        ...
        while ((list_node = singlylinkedlist_get_head_item(instance->waiting_to_send)) != NULL)
        {
            ...
            if (task != NULL)
            {
                task->on_event_send_complete_callback(task->message, GMESSENGER_SEND_COMPLETE_RESULT_MESSENGER_DESTROYED, (void*)task->context);
                free(task);
            }
        }
        ...
        destroy_config(instance->config);
...
GMESSENGER_HANDLE gmessenger_create(const GMESSENGER_CONFIG* messenger_config)
{
    GMESSENGER_HANDLE handle;

    if (validate_configuration(messenger_config) != 0)
    {
        handle = NULL;
        LogError("messenger_create failed (invalid configuration)");
    }
    else
    {
        ...
        else
        {
            ...
            instance->state = GMESSENGER_STATE_STOPPED;
            instance->message_sender_current_state = MESSAGE_SENDER_STATE_IDLE;
            instance->message_sender_previous_state = MESSAGE_SENDER_STATE_IDLE;
            instance->message_receiver_current_state = MESSAGE_RECEIVER_STATE_IDLE;
            instance->message_receiver_previous_state = MESSAGE_RECEIVER_STATE_IDLE;
            ...
            // Codes_SRS_IOTHUBTRANSPORT_AMQP_MESSENGER_09_008: [messenger_create() shall save a copy of `messenger_config->device_id` into `instance->config->device_id`]
            if ((instance->config = clone_configuration(messenger_config)) == NULL)
            {
                ...
                handle = NULL;
                LogError("messenger_create failed (failed cloning configuration)");
            }
            ...
            else
            {
                ...
                handle = (GMESSENGER_HANDLE)instance;
            }
        }

        if (handle == NULL)
        {
            gmessenger_destroy((GMESSENGER_HANDLE)instance);
        }
    }
